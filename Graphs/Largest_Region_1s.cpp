...
bool pathPossible(vector< vector<int>>& g, int &n, 
                vector< vector<bool>>& visited, int si, int sj, int& di, int& dj){
	...
	//coordinates for the 4 directions
    vector<int> x = {0,1,-1,0};
    vector<int> y = {1,0,0,-1};

    for(int k = 0; k<4; k++){
    	if(isSafe(si + x[k], sj + y[k], n, g) && !visited[si + x[k]][sj + y[k]]){

    		if(pathPossible(g, n, visited, si + x[k], sj + y[k], di, dj))
    			return true;
